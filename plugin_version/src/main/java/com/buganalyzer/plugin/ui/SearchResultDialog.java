...
public class SearchResultDialog extends DialogWrapper {
    ...
    private JLabel statusLabel; // Bound in form

    // Layered Pane similar to TextPanel
    ...
    public SearchResultDialog(Component parent, Project project, String projectPath, FileMetadata fileMetadata, String query, boolean isRegex, java.util.function.Consumer<SearchEngine.SearchResult> onNavigate) {
        super(parent, false);
    ...
    protected void dispose() {
        ...
        if (searchThread != null && searchThread.isAlive()) {
            searchThread.interrupt(); // Force interrupt
        }
        super.dispose();
    }
    ...
    private void performSearch() {
        ...
        searchThread = new Thread(() -> {
            ...
            for (File f : filesToSearch) {
                 if (isCancelled || Thread.currentThread().isInterrupted()) return; 
                 try {
                     List<SearchEngine.SearchResult> res = SearchEngine.searchFile(f, query, isRegex, true);
                     allResults.addAll(res);
                     if (allResults.size() > 5000) break;
                 } catch (Exception e) {
                     // e.printStackTrace(); 
                     // Interruption likely causes exception here (NIO closed etc), so we just exit
                     return;
                 }
            }
            ...
            SwingUtilities.invokeLater(() -> {
                ...
                statusLabel.setText("Found " + allResults.size() + " matches." + (allResults.size() >= 5000 ? " (Limited)" : ""));
            });
